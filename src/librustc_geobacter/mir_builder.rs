...
use rustc_middle::ty::{self, TyCtxt, Instance, InstanceDef, Const,
                       ConstKind, Ty};
    ...
    where Self: TyCtxtKernelInstance<'tcx> + TyCtxtConstBuilder<'tcx>,
{
    ...
    fn call_device_inst<F>(&self, mir: &mut mir::BodyAndCache<'tcx>, f: F)
    ...
    fn call_device_func<F>(&self, mir: &mut mir::BodyAndCache<'tcx>, f: F)
    ...
    fn call_device_inst_args<F>(&self, mir: &mut mir::BodyAndCache<'tcx>, f: F)
    ...
    fn call_device_func_args<F>(&self, mir: &mut mir::BodyAndCache<'tcx>, f: F)
    ...
    fn redirect_or_panic<F, G>(&self, mir: &mut mir::BodyAndCache<'tcx>,
                               msg: G, f: F)
        where F: FnOnce() -> Option<(Instance<'tcx>, Vec<Operand<'tcx>>)>,
              G: FnOnce() -> Operand<'tcx>,
    {
        ...
        let (callee, args, term_kind) = match f() {
            ...
            None => {
                ...
                let item = lang_items::PanicFnLangItem;
        ...
        debug!("mirgen intrinsic into {}", callee);

        let success = mir::BasicBlock::new(mir.basic_blocks().next_index().index() + 1);
        let fn_ty = callee.monomorphic_ty(tcx);
