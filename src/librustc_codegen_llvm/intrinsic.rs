...
impl IntrinsicCallMethods<'tcx> for Builder<'a, 'll, 'tcx> {
    fn codegen_intrinsic_call(
        ...
        let llval = match name {
            ...
            // Unlike below, we always require an ordering to make things simpler for
            // us.
            name if name.starts_with("atomic_scoped_fence_") => {
                use rustc_codegen_ssa::common::AtomicOrdering::*;

                let split: Vec<&str> = name.split('_').skip(3).collect();
                if split.len() < 2 {
