...
impl Step for GeobacterRuntime {
    ...
    fn make_run(run: RunConfig<'_>) {
        run.builder.ensure(GeobacterRuntime {
            compiler: run.builder.compiler(run.builder.top_stage, run.host),
            target: run.target,
        });
    }
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        if !builder.config.full_bootstrap || compiler.stage < 2 {
            builder.info(&format!(
                "Skipping stage{} geobacter-runtime",
                compiler.stage
            ));
            return;
        }
        if compiler.host != target {
            builder.info(&format!(
                "Skipping geobacter ({} != {})",
        ...
        let compiler_to_use = builder.compiler_for(compiler.stage, compiler.host, target);
        assert_eq!(compiler_to_use, compiler);
        if compiler_to_use != compiler {
            builder.ensure(GeobacterRuntime { compiler: compiler_to_use, target });
            builder.info(&format!("Uplifting stage1 geobacter-runtime ({} -> {})", compiler_to_use.host, target));

            // Even if we're not building std this stage, the new sysroot must
            // still contain the third party objects needed by various targets.
            copy_third_party_objects(builder, &compiler, target);

            builder.ensure(GeobacterRuntimeLink {
                compiler: compiler_to_use,
                target_compiler: compiler,
                target,
            });
        ...
        cargo
            .arg("--manifest-path")
            .arg(builder.src.join("src/libgeobacter/Cargo.toml"));

        builder.info(&format!(
            "Building stage{} geobacter-runtime artifacts ({} -> {})",
            compiler.stage, &compiler.host, target
        ));
        run_cargo(
            builder,
            cargo,
            vec![],
            &libgeobacter_stamp(builder, compiler, target),
            vec![],
            false,
        );
...
#[derive(Debug, Copy, Clone, PartialEq, Eq, Hash)]
struct GeobacterRuntimeLink {
    pub compiler: Compiler,
    pub target_compiler: Compiler,
    pub target: Interned<String>,
}

impl Step for GeobacterRuntimeLink {
    type Output = ();
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        let target = self.target;
        builder.info(&format!(
            "Copying stage{} goebacter from stage{} ({} -> {} / {})",
            target_compiler.stage, compiler.stage, &compiler.host, target_compiler.host, target
        ));
        let libdir = builder.sysroot_libdir(target_compiler, target);
        let hostdir = builder.sysroot_libdir(target_compiler, compiler.host);
...
impl Step for Rustc {
    ...
    fn run(self, builder: &Builder<'_>) {
        ...
        if compiler.stage == 2 {
            //builder.ensure(GeobacterRuntime { compiler, target, });
        }
...
/// Cargo's output path for the standard library in a given stage, compiled
/// by a particular compiler for the specified target.
pub fn libgeobacter_stamp(
    builder: &Builder<'_>,
    compiler: Compiler,
    target: Interned<String>,
) -> PathBuf {
    builder.cargo_out(compiler, Mode::Std, target).join(".libgeobacter.stamp")
}
