...
pub trait ShaderInterface {
    ...
    #[inline(always)]
    fn component(&self) -> u32 { Self::COMPONENT }
}

/// This type *must* be used for statics only.
#[cfg_attr(not(stage0), lang = "spirv_shader_input")]
#[repr(transparent)]
pub struct ShaderInput<T, const LOC: u32, const COMP: u32>(MaybeUninit<T>)
    where T: Copy;
impl<T, const LOC: u32, const COMP: u32> ShaderInput<T, {LOC}, {COMP}>
    where T: Copy,
{
    #[inline(always)]
...
impl<T, const LOC: u32, const COMP: u32> Deref for ShaderInput<T, {LOC}, {COMP}>
    where T: Copy,
{
    type Target = T;
    #[inline(always)]
...
impl<T, const LOC: u32, const COMP: u32> fmt::Debug for ShaderInput<T, {LOC}, {COMP}>
    where T: Copy + fmt::Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if platform().is_spirv() {
            f.debug_struct("Input")
                .field("location", &(LOC, COMP))
                .field("value", &self.0)
                .finish()
        } else {
            f.debug_struct("Input")
...
impl<T, const LOC: u32, const COMP: u32> ShaderInterface for ShaderInput<T, {LOC}, {COMP}>
    where T: Copy,
{
    const LOCATION: u32 = LOC;
    const COMPONENT: u32 = COMP;
}

/// This type *must* be used for statics only.
#[cfg_attr(not(stage0), lang = "spirv_shader_output")]
#[repr(transparent)]
pub struct ShaderOutput<T, const LOC: u32, const COMP: u32>(UnsafeCell<T>)
  where T: Copy;
impl<T, const LOC: u32, const COMP: u32> ShaderOutput<T, {LOC}, {COMP}>
    where T: Copy,
{
    #[inline(always)]
...
impl<T, const LOC: u32, const COMP: u32> Deref for ShaderOutput<T, {LOC}, {COMP}>
    where T: Copy,
{
    type Target = T;
    #[inline(always)]
...
impl<T, const LOC: u32, const COMP: u32> DerefMut for ShaderOutput<T, {LOC}, {COMP}>
    where T: Copy,
{
    #[inline(always)]
...
impl<T, const LOC: u32, const COMP: u32> fmt::Debug for ShaderOutput<T, {LOC}, {COMP}>
    where T: Copy + fmt::Debug,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if platform().is_spirv() {
...
impl<T, const LOC: u32, const COMP: u32> ShaderInterface for ShaderOutput<T, {LOC}, {COMP}>
    where T: Copy,
{
    const LOCATION: u32 = LOC;
    const COMPONENT: u32 = COMP;
